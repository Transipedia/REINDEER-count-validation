...
fit.tab <- cmp.tab.pos %>%
    dplyr::group_by(ID) %>%
    dplyr::summarise("alpha" = lm(formula = log1p(Sum.reindeer) ~ log1p(Kallisto.counts))$coefficients[2],
                     "beta" = lm(formula = log1p(Sum.reindeer) ~ log1p(Kallisto.counts))$coefficients[1],
                     "R2" = summary(lm(formula = log1p(Sum.reindeer) ~ log1p(Kallisto.counts)))$adj.r.squared,
                     "n_smp" = dplyr::n())
