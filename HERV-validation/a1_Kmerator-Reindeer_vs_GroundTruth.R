...
INDIR <- paste0(WKDIR, "data/HERV1000/")
OUTDIR <- paste0(WKDIR, "res_HERV1000/KmeratorReindeer/")
...
tab.telescope <- read.table(paste0(INDIR, "HERV1000_telescope.tsv"),
                            header = TRUE, sep = "\t") %>%
    pivot_longer(cols = -transcript, values_to = "Counts.true", names_to = "Sample.assay")

# Load and parse Reindeer table
tab.reindeer <- read.table(paste0(INDIR, "contigs_HERV1000_on_CCLE-24-cut.out"),
                           header = TRUE, sep = "\t") %>%
    dplyr::mutate(seq_name = str_remove_all(seq_name, ".contig[0-9]*")) %>%
...
tab.reindeer <- aggregate(tab.reindeer$Query.reindeer,
                          by = list(tab.reindeer$seq_name, tab.reindeer$Sample.assay),
                          FUN = function(qs) paste0(qs, collapse = ",")) %>%
    dplyr::rename(transcript = Group.1, Sample.assay = Group.2, Query.reindeer = x) %>%
...
cmp.tab <- merge(tab.telescope, tab.reindeer, by = c("transcript", "Sample.assay"))
...
ggsave((plt_lst[[1]] | plt_lst[[2]]) / (plt_lst[[3]] | plt_lst[[4]]),
       filename = paste0(OUTDIR, "SupplFig_KmeratorReindeer.4metrics.png"),
       width = 9, height = 5, dpi = 600)

# Dropout by Reindeer
all((cmp.tab$Mean.reindeer == 0) == (cmp.tab$Max.reindeer == 0))
write.table(cmp.tab[cmp.tab$Mean.reindeer == 0 & cmp.tab$Counts.true > 0, ],
            paste0(OUTDIR, "KmeratorReindeer_Dropped.tsv"),
